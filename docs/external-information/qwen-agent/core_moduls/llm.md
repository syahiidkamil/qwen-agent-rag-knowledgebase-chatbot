> **Source:** https://qwenlm.github.io/Qwen-Agent/en/guide/core_moduls/llm/
> **Fetched:** 2026-05-23
...
Please refer to the [Configuration](/Qwen-Agent/en/guide/core_moduls/get_started/configuration/) for detailed parameter configuration.
...
LLM classes uniformly use the `llm.chat(...)` interface to generate responses, supporting input of message lists, functions, and other parameters. For a detailed parameter list, see [BaseChatModel](/Qwen-Agent/en/qwen_agent/llm/base.py).

See a complete example of LLM usage at [Function Calling](/Qwen-Agent/en/examples/function_calling.py).
...
Qwen-Agent provides a mechanism for registering LLMs. In the [LLM Base Class](/Qwen-Agent/en/qwen_agent/llm/base.py), a uniform `llm.chat(...)` interface is implemented.
...
If the newly registered LLM does not support function calls, it can inherit from the [BaseFnCallModel](/Qwen-Agent/en/qwen_agent/llm/function_calling.py) class implemented in Qwen-Agent.
