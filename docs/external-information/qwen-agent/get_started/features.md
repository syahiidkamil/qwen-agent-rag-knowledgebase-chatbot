> **Source:** https://qwenlm.github.io/Qwen-Agent/en/guide/get_started/features/
> **Fetched:** 2026-05-23
...
- **Unified Agent Interface** High-level `Agent` base class with ready-to-use implementations (e.g., `Assistant`, `FnCallAgent`) for rapid development.
- **Advanced Tool Calling** Native support for **parallel**, **multi-step**, and **multi-turn** function/tool calls with automatic parsing and execution.
- **RAG** Efficient document QA over 1M+ tokens using hybrid RAG and agent-based decomposition—outperforming native long-context models in benchmarks.
- **Built-in Tools** Includes versatile tools out of the box:  `code_interpreter`: Execute Python code
