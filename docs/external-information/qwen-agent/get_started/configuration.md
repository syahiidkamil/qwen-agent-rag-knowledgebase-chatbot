> **Source:** https://qwenlm.github.io/Qwen-Agent/en/guide/get_started/configuration/
> **Fetched:** 2026-05-23
> **Note:** Mirror of upstream documentation for offline reference. Authoritative copy is the URL above.
...
|---|---|---|---|
| `model` | `str` | ✅ Yes | The model name to use. e.g., `'qwen3-max'`, `'qwen3-vl-plus'`, `'qwen3-omni-flash'`, `'qwen3-coder-plus'` |
| `model_type` | `str` | ✅ Yes | Specifies the model provider, and binds with model capability.Use Alibaba Cloud’s DashScope API:• `'qwen_dashscope'`: LLM, support Text —> Text. • `'qwenvl_dashscope'`: VLM, support Text/Image/Video —> Text. • `'qwenaudio_dashscope'`: Omni models, support Text/Image/Video/Audio —> Text. Use an OpenAI-compatible API: • `'oai'`: LLM, support Text —> Text. • `'qwenvl_oai'`: VLM, support Text/Image/Video —> Text. • `'qwenaudio_oai'`: Omni models, support Text/Image/Video/Audio —> Text. |
| `model_server` | `str` | ❌ Conditionally | Required only for OpenAI-compatible API, e.g., • `'http://localhost:8000/v1'`: local server, • `'https://dashscope.aliyuncs.com/compatible-mode/v1'`: OpenAI-compatible API of DashScope. |
| `api_key` | `str` | ❌ No | API key for authentication.• **DashScope**: Can be provided here or via the `DASHSCOPE_API_KEY` environment variable• **OpenAI-compatible API**: Can be provided here or via the `OPENAI_API_KEY` environment variable. |
...
|---|---|---|---|
| `max_input_tokens` | `int` | 90000 | The maximum context length of the agent, when the context exceeds this length, [context management](/Qwen-Agent/en/guide/core_moduls/context/) will be automatically performed. This parameter should be lower than the maximum input length supported by the model to ensure the normal operation of the agent. |
| `use_raw_api` | `bool` | `False` | Whether to use the model server’s native tool-call parsing (e.g., vLLM’s built-in parser). We recommend set `True` for models in the qwen3-coder, qwen3-max, and subsequent series. It will be changed to the default `True` in the future. |
| enable thinking | — | — | Enables “thinking mode” if supported by the model. Depends on the parameter protocol of the model service side. • DashScope: `enable_thinking=True` • OpenAI-compatible API of DashScope: `'extra_body': {'enable_thinking': True}` • OpenAI-compatible API of vLLM: `'extra_body': {'chat_template_kwargs': {'enable_thinking': True}}` |
...
```
- **Behavior**:  The system calls `MCPManager().initConfig(...)` to launch MCP services and auto-discover available tools.
...
- If multiple entries attempt to register a tool with the **same name**, the system: **Overwrites** any previous tool with the **latest occurrence** in the list.
